...
//============================
  ...
  UINT16   qq = 0;
  UINT16   new_PWM = 500;
  float   new_PWM_Emilie = 0x00;
  UINT16   not_new_PWM = 500;
  UINT16   dead_zone_limit = 250;
...
#define TIMESTAMP_VALUE 0.1f   //Also set in the Timer 1 params
  
  LED_ALL_OFF();
  
  MastStartPwm(not_new_PWM, new_PWM);
    ...
    while(rxBuf.buffer[0] == 0)
    {
      ...
      {
        LED_DEBUG2_ON;
        while(qq < dead_zone_limit){
          
        MastStartPwm(not_new_PWM, new_PWM); //Change these PWM value
        qq++;
        not_new_PWM -= qq;
        new_PWM += qq;
          ...
          if (oTimerReg)
          {
            ...
            new_PWM_Emilie = (float) new_PWM;
            //timeStamp += TIMESTAMP_VALUE;
            memcpy(&txBuf.buffer[0], &new_PWM_Emilie                  , 4);
