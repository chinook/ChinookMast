 ...
 * Returns        : None.
 *************************************************************/
void DRV_mgs(INT32 DRV, INT32 msg){
...
void Init_reg_Mast(void){
  ...
  Drive_Mast_Setup.Tor.TROQUE   = 0xFF;           //torque value, see equation of max current
...
void Init_reg_Pitch(void){
  Drive_Pitch_Setup.Ctl.ENABLE   = 0b1;            //0 = disable, 1 = enable
  Drive_Pitch_Setup.Ctl.RDIR     = 0b1;            //0 = use DIR pin, 1 = use inverse DIR PIN
  Drive_Pitch_Setup.Ctl.RSETEP   = 0b1;            //0 = nothing, 1 : give a step on stepper = STEP pin
  Drive_Pitch_Setup.Ctl.MODE     = 0b0000;         //000 = FULL step.... 1000 = 1/256, other reserved
  Drive_Pitch_Setup.Ctl.EXSTALL  = 0b0;            //0 = internal stall effect, 1 = external stall effect
  Drive_Pitch_Setup.Ctl.ISGAIN   = 0b00;           //gain AOP,  00=5, 01=10, 10=20, 11=40
  Drive_Pitch_Setup.Ctl.DTIME    = 0b00;           //dead time, 00=400ns, 01=450ns, 10=650ns, 11=850ns

  Drive_Pitch_Setup.Tor.TROQUE   = 0xFF;           //torque value, see equation of max current
  Drive_Pitch_Setup.Tor.SMPLTH   = 0b111;          //EMF sample : 000 = 50us, 001 = 100us, 010 = 200us, 011 = 300us, 100 = 400us, 101 = 600us, 110 = 800us, 111 = 1000us

  Drive_Pitch_Setup.Off.TOFF     = 0x80;           //set off time, increment of 500ns
  Drive_Pitch_Setup.Off.PWMMODE  = 0b0;            //0 = use internal indexer, 1 = use xINx inputs to control output

  Drive_Pitch_Setup.Bla.TBLANK   = 0x80;           //set current trip blank time
  Drive_Pitch_Setup.Bla.ABT      = 0b1;            //0 = disable adaptive blamking time, 1 = enable adaptive blamking time

  Drive_Pitch_Setup.Dec.TDECAY   = 0x80;           //increment of 500ns
  Drive_Pitch_Setup.Dec.DECMOD   = 0b101;          //000 : force slow decay
                                                  //001 : slow decay for increasing current
                                                  //010 : force fast decay
                                                  //011 : use mexed decay
                                                  //100 : slow decay for increasing current
  ...
  Drive_Pitch_Setup.Sll.SDTHR    = 0xFF;           //stall detec threshold, determined experimentaly
  Drive_Pitch_Setup.Sll.SDCNT    = 0b11;           //STALLn asserted after, 00 = 1 step, 01 = 2 step, 10 = 4 step, 11 = 8 step
  Drive_Pitch_Setup.Sll.VDIV     = 0b00;           //back EMF devided by, 00 = 32, 01 = 16, 10 = 8, 11 = 4

  Drive_Pitch_Setup.Dri.OCPTH    = 0b11;           //OCP threshold, 00 = 250mV, 01 = 500mV, 10 = 750mV, 11 = 1000mV,
  Drive_Pitch_Setup.Dri.OCPDEG   = 0b11;           //OCP deglitch time, 00 = 1 us, 01 = 2 us, 10 = 4 us, 11 = 8 us
  Drive_Pitch_Setup.Dri.TDRIVEN  = 0b11;           //Low-side gate drive time, 00 = 250 ns, 01 = 500 ns, 10 = 1 us, 11 = 2 us
  Drive_Pitch_Setup.Dri.TDRIVEP  = 0b11;           //High-side gate drive time, 00 = 250 ns, 01 = 500 ns, 10 = 1 us, 11 = 2 us
  Drive_Pitch_Setup.Dri.IDRIVEN  = 0b11;           //Low-side gate drive peak current, 00 = 100mA, 01 = 200mA, 10 = 300mA, 11 = 400mA
  Drive_Pitch_Setup.Dri.IDRIVEP  = 0b11;           //High-side gate drive peak current, 00 = 100mA, 01 = 200mA, 10 = 300mA, 11 = 400mA

  Drive_Pitch_Setup.Sta.OTS      = 0b0;            //0 = normale operation, 1 = overtemperature shutdown detected
  Drive_Pitch_Setup.Sta.AOCP     = 0b0;            //0 = normale operation, 1 = channel A overcurrent shutdown
  Drive_Pitch_Setup.Sta.BOCP     = 0b0;            //0 = normale operation, 1 = channel B overcurrent shutdown
  Drive_Pitch_Setup.Sta.APDF     = 0b0;            //0 = normale operation, 1 = channel A predriver fault
  Drive_Pitch_Setup.Sta.BPDF     = 0b0;            //0 = normale operation, 1 = channel B predriver fault
  Drive_Pitch_Setup.Sta.UVLO     = 0b0;            //0 = normale operation, 1 = overvoltage lockout
  Drive_Pitch_Setup.Sta.STD      = 0b0;            //0 = normale operation, 1 = stall detected
  Drive_Pitch_Setup.Sta.STDLAT   = 0b0;            //0 = normale operation, 1 = latched stall detect
